...
def transpose(tiles, width=None):
    ...
    tiles = sorted(enumerate(tiles), key= lambda x: x[0] % width)
...
def convert_2bpp_to_png(image, **kwargs):
    ...
    if width * height != px_length(image):
        ...
        if len(matches):
            width, height = sorted(matches, key= lambda dim: (dim[1] % 8 != 0, dim[0] + dim[1]))[0] # favor height
        else:
...
def convert_to_png_from_rom(offset, tiles, w=0):
    ...
    with open(f"gfx_{offset:0x}.png", 'wb') as f:
        w.write(f, px_map)
...
def main():
    ...
    if args.mode == 'png' and len(args.filenames) in [2, 3] and '.' not in args.filenames[0]:
        w = int(args.filenames[2]) if len(args.filenames) == 3 else 0
        convert_to_png_from_rom(int(args.filenames[0], 16), int(args.filenames[1], 16), w)
    else:

...
class BSSReader:
    ...
    def read_bss_sections(self, bss):
        ...
        for line in bss:
            ...
            split_line_upper = list(map(str.upper, split_line))

            if not line:
                pass

            elif 'RSRESET' in split_line_upper:
                rs = 0
            ...
            elif 'RB' in split_line_upper:
                index = split_line_upper.index('RB')
                name = split_line[:index][-1]
                value = rs
                if index == len(split_line_upper) - 1:
                    rs += 1
                else:
                    rs += int(split_line_upper[-1])
...
class WRAMProcessor(object):
    ...
    def __init__(self, config):
        ...
        if hasattr(self.config, "wram"):
            ...
            self.paths["wram"] = os.path.join(self.config.path, path + "ram/wram.asm")

        if hasattr(self.config, "hram"):
            self.paths["hram"] = self.config.hram
        else:
            self.paths["hram"] = os.path.join(self.config.path, path + "ram/hram.asm")
